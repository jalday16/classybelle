...
class Nav_Menu_Images_Admin extends Nav_Menu_Images {
	...
	public function __construct() {
		$this->plugin_dir = BASEL_THEME_DIR . '/inc/third-party/nav-menu-images/';
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'nmi-scripts', 'nmi_vars', array(
				'alert' => __( 'You need to set an image as a featured image to be able to use it as an menu item image', 'basel' )
			)
		);
		add_thickbox();
	...
	public function _wp_post_thumbnail_html( $content, $post_id ) {
		...
		if ( isset( $_REQUEST['thumb_was'] ) && -1 == $_REQUEST['thumb_was'] ) {
			$link_text = __( 'Change image', 'basel' );
			$ajax_nonce = wp_create_nonce( 'set_post_thumbnail-' . $post->ID );
			$remove_link = ' | <a href="#" data-id="' . $post->ID . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $post->ID . ');return false;">' . esc_html__( 'Remove image', 'basel' ) . '</a>';
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['nmihover'] = array(
			...
			'value'        => $is_hover,
			'helps'        => __( 'Should this image be used on hover', 'basel' ),
			'show_in_edit' => false
		);
		...
		$form_fields['nmiactive'] = array(
			'label'        => __( 'Used when active?', 'basel' ),
			'input'        => 'html',
			...
			'value'        => $is_active,
			'helps'        => __( 'Should this image be used when menu item is active', 'basel' ),
			'show_in_edit' => false
		);
    ...
    function custom_fields_save($menu_id, $menu_item_db_id, $args ) {
		//ar($_REQUEST);
		$fields = array('design', 'width', 'height', 'icon', 'event', 'label', 'colorscheme', 'opanchor', 'callbtn');
