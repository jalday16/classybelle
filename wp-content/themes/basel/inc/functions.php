...
if( ! function_exists( 'basel_body_class' ) ) {
	function basel_body_class( $classes ) {
		...
		//Off canvas sidebar
		if( $hide_sidebar_mobile && ( basel_woocommerce_installed() && ( is_shop() || is_product_category() ) ) || $main_sidebar_mobile && ( ! basel_woocommerce_installed() || ( ! is_shop() && ! is_product_category() ) ) ) {
		...
		if ( ! empty( $attr['image_poster_switch'] ) ) {
			...
			$output = preg_replace_callback('/wpb_video_wrapper.*?>/',
				function ( $matches ) use( $image ) {
				   return strtolower( $matches[0] . '<div class="basel-video-poster-wrapper"><div class="basel-video-poster" style="background-image:url(' . $image . ')";></div><div class="button-play"></div></div>' );
				}, $output );
		}
		return $output;
	}
...
if( ! function_exists( 'basel_get_attachment_placeholder' ) ) {
	function basel_get_attachment_placeholder( $id, $size ) {
		...
		if( basel_get_opt( 'lazy_generate_previews' ) ) {
			...
			$img = $uploaded['url'];
			if( basel_get_opt( 'lazy_proprtion_size' ) ) {
