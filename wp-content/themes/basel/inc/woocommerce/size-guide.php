        ...
        if ( isset( $sguide_post_id[0] ) && !empty( $sguide_post_id[0] ) ){
            ...
            if ( $terms ) {
                foreach( $terms as $term ){
                    if ( get_woocommerce_term_meta( $term->term_id, 'basel_chosen_sguide' ) ) {
                        $sguide_post_id = get_woocommerce_term_meta( $term->term_id, 'basel_chosen_sguide' );
                    }else{
        ...
        ?>
                ...
                <style type="text/css" data-type="vc_shortcodes-custom-css">
                    ...
                    <?php if ( ! empty( $basel_shortcodes_custom_css ) ): ?>
                        <?php echo $basel_shortcodes_custom_css; ?>
                    <?php endif ?>
                </style>
...
if( ! function_exists( 'basel_sguide_save_category' ) ) {
    function basel_sguide_save_category( $post_id ) {
        if ( isset( $_POST['basel_sguide_category'] ) ) {
            $selected_sguide_category = $_POST['basel_sguide_category'];
            ...
            foreach( $terms as $term ){
                if ( !in_array( $term->term_id, $selected_sguide_category ) ) {
                    if ( $post_id == get_woocommerce_term_meta( $term->term_id, 'basel_chosen_sguide' ) ) {
    ...
    function basel_sguide_category_template( $post ) {
        ...
        <ul>
            <?php foreach ( $sguide_cat_list as $sguide_cat ): ?>
                ...
                <li>
                    <input type="checkbox" name="basel_sguide_category[]" value="<?php echo $sguide_cat->term_id; ?>" <?php echo $checked; ?>>
