...
if( ! function_exists( 'basel_product_sale_countdown' ) ) {
    function basel_product_sale_countdown() {
		...
		if ( apply_filters( 'basel_sale_countdown_variable', false ) && $product->get_type() == 'variable' && $variations = $product->get_available_variations() ) {
...
if( ! function_exists( 'basel_shop_vc_css' ) ) {
	function basel_shop_vc_css() {
		...
		if ( ! empty( $shop_custom_css ) ) {
			echo '<style type="text/css" data-type="vc_shortcodes-custom-css">' . $shop_custom_css . '</style>';
		}
	}
					...
					<div class="basel-sticky-btn-cart">
						...
						<?php else: ?>
							<a href="<?php echo esc_url( $product->add_to_cart_url() ); ?>" class="basel-sticky-add-to-cart button alt">
								<?php echo $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'basel' ) : $product->single_add_to_cart_text(); ?>
							</a>
						<?php endif; ?>
						<?php 
...
if ( ! function_exists( 'basel_sticky_sidebar_button' ) ) {
	function basel_sticky_sidebar_button() {
		$sidebar_class = basel_get_sidebar_class();
		if ( strstr( $sidebar_class, 'col-sm-0' ) || basel_maintenance_page() ) return;

		if ( ( basel_woocommerce_installed() && ( is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) && basel_get_opt( 'sticky_filter_button' ) && ( basel_get_opt( 'shop_hide_sidebar_mobile' ) || basel_get_opt( 'shop_hide_sidebar_tablet' ) ) ) ) {
			echo '<a href="#" class="basel-sticky-sidebar-opener shop-sidebar-opener">' . basel_get_svg_content( 'filter-icon' ) . '</a>';
		} elseif ( basel_get_opt( 'hide_main_sidebar_mobile' ) && ( ! basel_woocommerce_installed() || ( ! is_shop() && ! is_product_category() && ! is_product_tag() && ! is_product_taxonomy() ) ) ) {
			echo '<a href="#" class="basel-sticky-sidebar-opener"></a>';
		}
	}
